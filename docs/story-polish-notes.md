...
IBM's track rule is a process gate: projects that do not demonstrate Bob usage fail
regardless of code quality. Sightread was built through Bob Shell 2.0.2 over the Agent
Client Protocol. Seven sessions live in `bob/transcripts/`. Bob's own SQLite ledger at
`~/.bob/db/bob.db` recorded **22,158** output tokens across six build sessions.
`/api/bob` on the hosted service reads those transcripts off disk rather than
restating them. Two sessions exist because Bob's work went back to Bob (an invented
trailing silence, a test that could not fail) rather than being patched by hand. File
map and session ids are under "How we built it" below.

## Inspiration

### The shape of the problem

Alex, a re-recording mixer at a post house, receives a describer's script and a
narrator's recordings, lays them into the silences, and finds a line runs 400 ms past
the point where dialogue comes back. That round trip is the whole cost. Description
generators write lines. They do not measure the silence, speak the take, then reject
the ones that overrun. Sightread does: **7 of 19** takes rejected on measurement across
two Night Tide runs, including a **15 ms** overrun a mixer by ear would let through.
Honest limit: Sightread hands back WAVs and a conform report; it does not yet place
lines into a mixed print.
